...
Future<void> main(List<String> arguments) async {
  try {
    ...
    stderr.writeln('Network error while fetching comparison data: ${error.message}');
    exitCode = 1;
...
void _printUsage() {
  ...
  stdout.writeln('  --start-year <year>    Highest season year to include. Defaults to $_defaultStartYear.');
  stdout.writeln('  --end-year <year>      Lowest season year to include. Defaults to $_defaultEndYear.');
  stdout.writeln('  --output-dir <path>    Directory for the JSON file. Defaults to $_defaultOutputDirectory.');
  stdout.writeln('  --file-name <name>     Output file name. Defaults to $_defaultFileName.');
  stdout.writeln('  --help                 Show this help message.');
  stdout.writeln('');
  stdout.writeln('Example:');
  stdout.writeln('  dart run fetch_driver_comparison_data.dart --start-year 2025 --end-year 2017');
}
    ...
    for (var index = 0; index < arguments.length; index++) {
      ...
      if (argument == '--start-year') {
        if (index + 1 >= arguments.length) {
          throw const DriverComparisonCliUsageException('Missing value for --start-year.');
        }
      ...
      if (argument == '--end-year') {
        if (index + 1 >= arguments.length) {
          throw const DriverComparisonCliUsageException('Missing value for --end-year.');
        }
      ...
      if (argument == '--output-dir') {
        if (index + 1 >= arguments.length) {
          throw const DriverComparisonCliUsageException('Missing value for --output-dir.');
        }
      ...
      if (argument == '--file-name') {
        if (index + 1 >= arguments.length) {
          throw const DriverComparisonCliUsageException('Missing value for --file-name.');
        }
...
class DriverComparisonDataFetcher {
  ...
  Future<Map<String, dynamic>> _fetchYear(int year) async {
    final standingsUri = Uri.parse('$_jolpicaBaseUrl/$year/driverStandings.json');
    final standingsResponse = await _getWithRetry(standingsUri);

    if (standingsResponse.statusCode != 200) {
      throw HttpException('Failed to fetch driver standings for $year (HTTP ${standingsResponse.statusCode}).');
    }

    final standingsData = jsonDecode(standingsResponse.body) as Map<String, dynamic>;
    final standingsLists = standingsData['MRData']?['StandingsTable']?['StandingsLists'] as List? ?? const <dynamic>[];
    ...
    final driverStandings = finalStandings['DriverStandings'] as List? ?? const <dynamic>[];
    ...
    final seasonResultsUri = Uri.parse('$_jolpicaBaseUrl/$year/results.json?limit=1000');
    final seasonQualifyingUri = Uri.parse('$_jolpicaBaseUrl/$year/qualifying.json?limit=1000');
    ...
    if (seasonResultsResponse.statusCode != 200) {
      throw HttpException('Failed to fetch season race results for $year (HTTP ${seasonResultsResponse.statusCode}).');
    }
    if (seasonQualifyingResponse.statusCode != 200) {
      throw HttpException('Failed to fetch season qualifying results for $year (HTTP ${seasonQualifyingResponse.statusCode}).');
    }

    final seasonResultsData = jsonDecode(seasonResultsResponse.body) as Map<String, dynamic>;
    final seasonQualifyingData = jsonDecode(seasonQualifyingResponse.body) as Map<String, dynamic>;
    final resultEntriesByDriver = _buildResultEntriesByDriver(seasonResultsData);
    final qualifyingEntriesByDriver = _buildQualifyingEntriesByDriver(seasonQualifyingData);
    ...
    for (final entry in driverStandings.whereType<Map>()) {
      ...
      final driverName = '${driverData['givenName'] ?? ''} ${driverData['familyName'] ?? ''}'.trim();
      ...
      final stats = _buildDriverYearStats(
        driverId: driverId,
        points: points,
        resultEntries: resultEntriesByDriver[driverId] ?? const <Map<String, dynamic>>[],
        qualifyingEntries: qualifyingEntriesByDriver[driverId] ?? const <Map<String, dynamic>>[],
  ...
  Map<String, dynamic> _buildDriverYearStats({
    ...
    final sortedResultEntries = [...resultEntries]
      ..sort(
        (left, right) => (left['round'] as int).compareTo(right['round'] as int),
      );
    ...
    final raceNameFromResults = <int, String>{
      for (final entry in sortedResultEntries) entry['round'] as int: entry['raceName'] as String,
    };
    ...
    for (final resultEntry in sortedResultEntries) {
      ...
      if (finishPosition != null) {
        ...
        highestFinish = highestFinish == null ? finishPosition : _minInt(highestFinish, finishPosition);
      }
    ...
    for (final round in sortedRounds) {
      pointsHistory.add(<String, dynamic>{
        'round': round,
        'raceName': raceNamesByRound[round] ?? raceNameFromResults[round] ?? 'Round $round',
    ...
    for (final qualifyingEntry in sortedQualifyingEntries) {
      ...
      final qualifyingPosition = int.tryParse(qualifyingResult['position']?.toString() ?? '');
      if (qualifyingPosition != null && qualifyingPosition > 0) {
        if (qualifyingPosition == 1) {
          poles += 1;
        }
        highestGrid = highestGrid == null ? qualifyingPosition : _minInt(highestGrid, qualifyingPosition);
      }
    }
  ...
  Future<Map<int, String>> _fetchRaceNamesByRound(int year) async {
    ...
    final scheduleData = jsonDecode(scheduleResponse.body) as Map<String, dynamic>;
  ...
  Future<Map<String, Map<int, double>>> _fetchPointsByRound({
    ...
    for (var round = 1; round <= finalRound; round++) {
      ...
      for (final entry in driverStandings.whereType<Map>()) {
        ...
        final points = double.tryParse(entry['points']?.toString() ?? '') ?? 0.0;
        pointsByRound.putIfAbsent(driverId, () => <int, double>{})[round] = points;
      }
    }
...
Map<String, List<Map<String, dynamic>>> _buildResultEntriesByDriver(
  ...
  for (final race in resultRaces.whereType<Map>()) {
    ...
    for (final resultEntry in results.whereType<Map>()) {
      final driverData = resultEntry['Driver'] as Map? ?? const <String, dynamic>{};
      final driverId = driverData['driverId']?.toString();
      if (driverId == null || driverId.isEmpty) {
        continue;
      }

      entriesByDriver.putIfAbsent(driverId, () => <Map<String, dynamic>>[]).add(
        <String, dynamic>{
          'round': round,
          'raceName': raceName,
